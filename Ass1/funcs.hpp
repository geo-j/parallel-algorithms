...
pair<vector<int>, vector<int>> local_sieve(int s, int b, int N) {
    int list_len =  N / b + 1;
    int sqrtN = sqrt(N)+1;
    int number = s;
...
pair<vector<int>, vector<int>> b_coprimes(int b, int pid, int p) {
...
map<int, int> inverse_dict (int b, int s, vector<int> k_list) {
...
void remove_multiples(int s, int b , int a, int sa, vector<int>& local_prime_bools,  map<int, int> inverse_dict_s) {
